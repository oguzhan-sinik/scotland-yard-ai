...
public class minimaxAlg {
    ...
    // We have constants here to more easily control our alg.

    private static final int DEPTH_STANDARD = 10;
    private static final int ESCAPE_ROUTE_PENALTY = 30;
    private static final int DANGER_PENALTY = 5000;
    private static final int DANGER_PENALTY_PER_DETECTIVE = 3000;
    private static final int MIN_DIST_PENALTY_ON_REVEALS = 500;
    private static final int MIN_DIST_MARGIN_TO_CLOSEST_DET = 200;
    private static final int EXTREME_SITUATION = -10000;
    private static final int DOUBLE_MOVE_PEN = 1500;
    private static final int DOUBLE_MOVE_PEN_CLOSE = 600;
    private static final int DOUBLE_MOVE_PEN_SO_CLOSE = 100;
    private static final int SAME_NODE_PENALTY = 3000;
    private static final int DET_CLOSE_ON_REVEAL = 4000;
    private static final int DET_FAR_ON_REVEAL = 200;
    private static final int SECRET_PENALTY_WHEN_NOT_REVEAL = 500;
    private static final int SECRET_PEN_WHEN_DET_CLOSE_AND_NOT_REVEAL = 1500;
    ...
    public static Integer nodeScore(Board board, int mrXLoc, List<Integer> detectiveLocs) {
        ...
        totalScore += escapeRouteCount(board, mrXLoc, detectiveLocs) * ESCAPE_ROUTE_PENALTY;
...
// we calculate the total score by looking at how far each detective is from mrX
    ...
    private static Integer totalScoreBasedOnDetectiveLocations (Board board, int[] distFromMrX, List<Integer> detectiveLocs) {
        ...
        */
        ...
        if (dangerCount > 0) totalScore -= DANGER_PENALTY + (dangerCount * DANGER_PENALTY_PER_DETECTIVE);

        // on reveal rounds we penalise being close to detectives

        if (wasRevealRound(board) && minDist <= 3) totalScore -= minDist * MIN_DIST_PENALTY_ON_REVEALS;

        // we also give a bonus for the closest detective to add a margin

        if (minDist != Integer.MAX_VALUE) totalScore += minDist * MIN_DIST_MARGIN_TO_CLOSEST_DET;

        return totalScore;
    }

    static boolean wasRevealRound(Board board) { // for being extra carefully if the round is the reveal round

        int nextRound = board.getMrXTravelLog().size();
        if (nextRound == 0) return false;

        List<Boolean> rounds = board.getSetup().moves;
        return nextRound < rounds.size() && rounds.get(nextRound - 1);
    }
    ...
    static Integer ticketIntelligence (Board board, Move move, Integer score, int mrXLoc, List<Integer> detectiveLocs) {
        ...
        // after reveal rounds, if detectives are close, using secret tickets is smart move, but otherwise it is not.

        if (wasRevealRound(board)) {

            if (usesSecret && minDist <= 2) score += DET_CLOSE_ON_REVEAL;
            else if (usesSecret && minDist > 3) score -= DET_FAR_ON_REVEAL;

        } else {

            score -= countSecretTicket(move) * SECRET_PENALTY_WHEN_NOT_REVEAL;
            if (minDist > 3) score -= countSecretTicket(move) * SECRET_PEN_WHEN_DET_CLOSE_AND_NOT_REVEAL; // leave secret tickets for next rounds

        }

        int destination = destination(move);
        if (mrXLoc == destination) score -= SAME_NODE_PENALTY; // we don't want to stay in same nodes
        if (minDist < 2) score += graph.degree(destination) * 8; // penalty for nodes that have better connectivity, we can escape more easily

        // double moves only we are in extremely danger

        if (isDoubleMove(move)) {

            if (minDist <= 2) score -= DOUBLE_MOVE_PEN_SO_CLOSE;
            if (minDist <= 4) score -= DOUBLE_MOVE_PEN_CLOSE;
            else score -= DOUBLE_MOVE_PEN;
...
// checks if a move is double move using visitor pattern
    ...
    static Integer treeDepth (long time) {
        return DEPTH_STANDARD + ((int) time / 15);
    }
    ...
    @Nonnull static Integer minimaxMrXTurn (Board board, int depth, int mrXLoc, List<Integer> detectiveLocs, int alpha, int beta) {
        ...
        if (bestScore == Integer.MIN_VALUE) return EXTREME_SITUATION;
        return bestScore;
    }
    ...
    @Nonnull static Integer minimaxDetectiveTurn (Board board, int depth, int mrXLoc, List<Integer> detectiveLocs, int alpha, int beta) {
        ...
        if (potNewDetLocs == null) return EXTREME_SITUATION; // MrX got caught
